...
AC_API_BASE = "https://api.actransit.org/transit/"
AC_TRANSIT_KEY = "2EBBBEE2332A6A4A79963F14CC0D1381"
...
def on_intent(intent_request, session):
    ...
    elif intent_name == "TestDynamo":
        return testDynamo()
    elif intent_name == "nextBusRequest":
        return nextBusRequest(intent)
...
def nextBusRequest(intent):
    ...
    reprompt_text = "I'm not sure which stop you wanted bus times for. " \
                    "Try asking about 51399 or 59229 for example."
    should_end_session = False

    if "StopID" in intent["slots"]:
        stopID = intent["slots"]["StopID"]["value"]

        card_title = "Bus departures from " + str(stopID)

        response = urllib2.urlopen(AC_API_BASE + "stops/" + str(stopID) + "/predictions/?token=" + AC_TRANSIT_KEY)
        bus_departures = json.load(response)
        if len(bus_departures) > 0:
            bus = bus_departures[0]
            speech_output = "A " + bus["RouteName"] + " bus is arriving at " + milToPeople(bus["PredictedDeparture"][-8:]);
        # for bus in bus_departures:
        #     speech_output += "Route number " + bus["RouteName"];
